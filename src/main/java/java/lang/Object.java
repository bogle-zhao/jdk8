...
public class Object {
     ...
     *
     * <p>
     *  唤醒正在等待此对象的监视器的单个线程。如果多个线程正在等待这个对象,其中一个被选择被唤醒。选择是任意的,由实现的自由裁量权决定。一个线程通过调用{@code wait}方法来等待对象的监视器。
     * <p>
     * 唤醒的线程将不能继续执行,直到当前线程放弃对此对象的锁定。唤醒的线程将以通常的方式与可能主动竞争以在该对象上同步的任何其他线程竞争;例如,被唤醒的线程在下一个线程锁定此对象时没有可靠的特权或缺点。
     * <p>
     *  此方法只应由作为此对象的监视器所有者的线程调用。线程通过以下三种方式之一成为对象的监视器的所有者：
     * <ul>
     *  <li>通过执行该对象的同步实例方法。
     * <li>通过执行在对象上同步的{@code synchronized}语句的正文。
     ...
     *  当前线程必须拥有此对象的监视器。
     * <p>
     *  此方法使当前线程(称为<var> T </var>)将自身置于此对象的等待集合中,然后放弃对此对象的任何和所有同步声明。
     ...
     * </ul>
     * 线程<var> T </var>然后从此对象的等待集合中删除,并重新启用线程调度。
     ...
     * 
     * <p>
     *  请注意,{@code wait}方法将当前线程放入此对象的等待集合中,只解锁此对象;当前线程可以同步的任何其他对象在线程等待时保持锁定。
     * <p>
     *  此方法只应由作为此对象的监视器所有者的线程调用。有关线程成为监视器所有者的方式的说明,请参阅{@code notify}方法。
     * 
     * 
     * @param      timeout   the maximum time to wait in milliseconds.      最大等待时间，单位：毫秒
     ...
     * @param      nanos      additional time, in nanoseconds range     纳秒
     *                       0-999999.
     * @throws  IllegalArgumentException      if the value of timeout is
