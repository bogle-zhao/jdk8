 ...
 *
 * <p>
 *  <i>Thread</i>是程序中的执行线程。 Java虚拟机允许应用程序具有并发运行的多个执行线程。
 * <p>
 *  每个线程都有一个优先级。具有较高优先级的线程优先于具有较低优先级的线程执行。每个线程可以标记为守护程序，也可以不标记为守护程序(后台线程)。
 * 当在一个线程中运行的代码创建一个新的<code> Thread </code>对象时,新线程的优先级最初设置为等于创建线程的优先级,并且当且仅当创建线程为守护进程，被创建的线程才是守护进程
 * <p>
 ...
 *  <li>所有非守护线程的线程都已经死亡,或者是通过调用<code> run </code>方法返回或抛出异常传播<code> run </code>方法。
 * </ul>
 * <p>
 ...
 * 。
 * <p>
 *  每个线程都有一个名称用于识别。多个线程可以具有相同的名称。如果在创建线程时未指定名称,则会为其生成新名称。
 * <p>
...
class Thread implements Runnable {
   ...
   /**
     ...
     * @param target the object whose run() method gets called  被执行的目标
     * @param name the name of the new Thread   线程名字
     * @param stackSize the desired stack size for the new thread, or
     *        zero to indicate that this parameter is to be ignored.    新的线程所期望的stack（栈的大小）size，或者0，0表示这个参数被忽略
